...
{
	OwnerCharacter = OwnerCharacter == nullptr ? Cast<AWOGBaseCharacter>(GetOwner()) : OwnerCharacter;
	...
	else if (Health + Value <= 0.f)
	{
		...
		Health = 0.f;
		OwnerCharacter->Server_SetCharacterState(ECharacterState::ECS_Elimmed, InstigatedBy);
